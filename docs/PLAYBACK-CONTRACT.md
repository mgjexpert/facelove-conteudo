...
O catálogo `/v1/catalog` fornece apenas `key`, `mediaType`, `mimeType`, `title`, `caption`, `visibility`, `thumbnailReference`, `packId` e os packs com IDs de assets internos. Não devolve `externalId`, URL de origem nem chaves. Os 55 assets do primeiro manifest MEGA tinham visibilidade `access_link`; o catálogo multiálbum atual enumera os ficheiros disponíveis e o frontend aplica as quotas de cada convite no servidor. A identificação do pack não concede playback.
...
Para produção, `MEDIA_GATEWAY_URL` aponta para o **segundo projeto Vercel** do gateway por HTTPS. O frontend valida o convite, a quota e a visibilidade do asset antes de pedir o stream ao gateway. O token Bearer passa apenas de servidor para servidor; consulte [VERCEL-GATEWAY.md](VERCEL-GATEWAY.md). O piloto posterior acrescentou convites persistidos em `access_links`, revogação individual, auditoria de ativação em `access_events` e RLS. A autorização de cada pedido é revalidada antes de servir media.
